describe('Escenarios datos Presudo-Aleatorios', () => {
    ...
    it('Escenario 111: Pseudo-Aleatorio Create member with name and email only', () => {
      ...
      cy.fixture('configuration').then((configuration)  => {
        ...
        cy.request("GET", "https://my.api.mockaroo.com/members.json?key=a915acb0", {
          ...
          cy.get('input[id="member-name"]').type(datapool.body[0].name_member, {force: true})
          cy.wait(2000)
          cy.get('input[id=member-email]').type(datapool.body[0].email_member, {force: true})
          cy.wait(2000)
