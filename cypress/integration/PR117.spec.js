describe('Escenarios datos Presudo-Aleatorios', () => {
    ...
    it('Escenario 117: Pseudo-Aleatorio Create member with email and label only', () => {
      ...
      cy.fixture('configuration').then((configuration)  => {
        ...
        cy.request("GET", "https://my.api.mockaroo.com/members.json?key=a915acb0", {
          ...
          cy.wait(2000)
          cy.get('input[id=member-email]').type(datapool.body[6].email_member, {force: true})
          cy.wait(2000)
          cy.get('.ember-power-select-trigger-multiple-input').type(datapool.body[6].label_member, {force: true})
          cy.wait(2000)
